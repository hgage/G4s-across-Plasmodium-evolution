...
def cleanlocations():
    """Split the location in the SepRecord into a list so that the values for "start", "end", and "strand" can be easily
    accessed.
...
PQS_dict = defaultdict(list)  # store data for PQSs on the sense strand
PQS_dict_RC = defaultdict(list)  # store data for PQSs on the antisense strand
def cleanPQSfile():
    ...
    """
    with open("/Users/HunterGage/Desktop/G4 Hunter_Analysis/P.falciparum_G4analysis.csv") as csvfile:
        ...
        for row in reader:
            ...
            if "P" in row["Chromosome"] and not "RC" in row["Chromosome"]: # if the chromosome is a contig and not RC
                if row["Telomere?"] == "Yes": # filter out telomeres
                    ...
                    PQS_dict[str(row["Location"])].append([str(row["Chromosome"]), int(row["LENGTH"])])
...
def sample_from_normal_distribution():
    """Generate and sample from a distribution of var/pir gene lengths that is normally distributed arond the mean, and
    store these lengths in random_var_sizes. Note that a size of 1 million simulations can cause the program to take
    ~3-5 minutes to execute. A size of 100,000 simulations does not drastically change the results, and takes
...
def get_chromosome_sizes():
    ...
    chromosome number (eg: "10"). If the chromosome is a contig, the chromosomeID is the full fasta name (eg: PADLG01_00_01).
    """
    for seq_record in SeqIO.parse(
        ...
        if "00_" in chromosome:  # if the "chromosome" is a contig, chromosomeID should be full fasta name
                                 # (eg: PADLG01_00_01)
            chromosome = seq_record.id
        elif int(chromosome[0]) == 0 and "_" in chromosome:  # if the chromosome number is < 10, chromosome ID should be
                                                             # the chromosome nubmer without leading 0 (eg: "1")
            chromosome = chromosome[1:chromosome.find("_")]
        elif int(chromosome[0]) == 0 and not "_" in chromosome:
            chromosome = chromosome[1]
        elif int(chromosome[0]) == 1 and "_" in chromosome:  # if the chromosome number is >=`0, chromosomeID should be
...
def calculate_total_genome_size():
    """Add the lengths of all of the chromosomes/contigs to calculate the toal size o the genome in bp."
...
chromosomes_picked = {}  # track the chromosomes picked for testing purposes
...
def g4s_on_chromosome(chromosome):
    ...
    :return: a dictionary storing PQS start locations, chromosomeIDs, and PQS lengths for all PQSs on the chromosome
    """
...
def find_nearest_g4(chromosome, position, length, start_end):
    """Compute the distance to the nearest PQS when given the chromosome, position, and length of a var/pir gnee. If a
    ...
    :param start_end: "start" if the "position" refers to the start o the var/pir gene, and "end" if the "position"
    ...
    for g4 in g4_data:  # populate the list of PQS start sites for the chosen chromosome
        for item in g4_data[g4]:
            g4_start_sites.append(g4)

    for g4 in g4_data:  # populate the list of PQS end sites for the chosen chromosome
        for item in g4_data[g4]:
            g4_end_sites.append(int(g4) + int(item[1]))  # PQS end site = PQS start site + PQS length
...
simulated_distances = []  # store distances between simulated var/pir genes and their nearest PQS
...
def do_simulation():
    ...
    """
    for var_gene in random_var_sizes:  # repeat for each simulated var gene
        ...
        position = random.randint(0, int(chromosome_sizes[int(chromosome_number)-1]))  # choose a random position on the
                                                                                       # chosen chromosome
        if var_gene < 0:  # very rarely, the simulation might generate a negative var/pir gene size
                          # if this occurs, take the "start" position to be position + length
            nearest_from_start_of_var = find_nearest_g4(chromosome_ID, (position + var_gene), abs(var_gene), "start")
            ...
            nearest_from_end_of_var = find_nearest_g4(chromosome_ID, position, abs(var_gene), "end")[0]
            nearest = min(nearest_from_start_of_var, nearest_from_end_of_var)
            simulated_distances.append(nearest)

        elif var_gene >= 0:
            nearest_from_start_of_var = find_nearest_g4(chromosome_ID, position, abs(var_gene), "start")
            if nearest_from_start_of_var is None:  # if no PQSs were found on chromosome, continue to next var/pir gene
                continue
            else:
                nearest_from_start_of_var = nearest_from_start_of_var[0]
            nearest_from_end_of_var = find_nearest_g4(chromosome_ID, (position + abs(var_gene)), abs(var_gene), "end")[0]
            nearest = min(nearest_from_start_of_var, nearest_from_end_of_var)
...
actual_distances = []  # store distances between actual var/pir genes and their nearest PQS
def do_my_data():
    ...
    for seq in seqs_info:
        ...
        if "erythrocyte membrane protein 1" in seq.description[2].lower():
            ...
            if "_00_" in chromosome:  # if chromosome is a contig, leave chromosomeID as is (eg: "PADLG01_00_01")
                ...
                continue
            chromosome = chromosome[chromosome.find("_")+1:]  # if chromosome is not a ocntig, represent the ID as
                                                              # a number (eg: "1" or "11")
    ...
    for var_gene in var_genes:
        for item in var_genes[var_gene]:
            ...
            if nearest_from_start_of_var is None:  # if no PQSs on chromosome, skip the gene
            ...
            nearest_from_end_of_var = find_nearest_g4(item[0]["chromosome"], (int(var_gene) + int(item[0]["length"])),
                                                      item[0]["length"], "end")[0]
            nearest = min(nearest_from_start_of_var, nearest_from_end_of_var)
            #if nearest > 1000000:  # use if exluding outliers
                #continue
