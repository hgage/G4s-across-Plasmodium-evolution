...
with open("/Users/HunterGage/Desktop/QGRS Mapper_Analysis/P.praefalciparum_analysis.csv", 'r') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        if row["Coding, upstream, or downstream?"].lower() == "none":
            if "RC" in row["Chromosome"]:
                    G4_locations[str(row["RC_Location Calculated"])].append(["None", "None"])
            else:
                G4_locations[str(row["Location"])].append(["None", row["Gene Annotation"]])
        elif "RC" in row["Chromosome"]:
            G4_locations[str(row["RC_Location Calculated"])].append([row["Nearest gene ID"], row["Gene Annotation"]])
        else:
...
def find_position_in_gene():
    ...
    for G4 in G4_locations:  # loop over all PQSs
        for entry in G4_locations[G4]:
            if entry[0] == "None":
                summary_data[G4].append([{"Nearest gene ID": entry[0], "Position in gene": "None", "Gene annotation": "None"}])
            else:
                for seq in seqs_info:  # loop over all coding sequences
                    if entry[0] == seq.description[0]:  # nearest gene ID of PQS matches the gene ID of coding sequence
                        if seq.description[3][2][:seq.description[3][2].find("\)")] == "+":  # gene on sense strand
                            ...
                            summary_data[G4].append([{"Nearest gene ID": entry[0], "Position in gene": position_in_gene, "Gene annotation": entry[1]}])
