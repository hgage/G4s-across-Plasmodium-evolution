"""This script compares the concordance of G4 Hunter and experimental G4-seq data by searching for instances where
the locations of PQSs predicted by G4 Hunter overlap with a G4-seq window."""
...
def cleanPQSfile():
    ...
    with open("/Users/HunterGage/Desktop/G4 Hunter_Analysis/P.falciparum_G4analysis.csv") as csvfile:
        ...
        for row in reader:
            if float(row["SCORE"]) > 0:  # if the PQS is on the sense strand
                PQS_dict[str(row["Location"])].append([str(row["Chromosome"]), int(row["LENGTH"])])
            elif float(row["SCORE"]) < 0:  # if the PQS is on the reverse complement strand
                PQS_dict_RC[str(row["Location"])].append([str(row["Chromosome"]), int(row["LENGTH"])])
cleanPQSfile()
...
def compare_lists():
    """For each PQS found by G4 Hunter, determine whether its location overlaps with a G4-seq window, and store this
            ...
            for G4_seq in G4seqs:  # loop over all of the G4-seq windows on the sense strand
                if item[0] == G4_seq["chromosome"]:  # only consider PQSs and G4-seq windows on the same chromosome
                    if int(G4_seq["start"]) <= int(PQS) <= int(G4_seq["end"]):
                        data.append(str(PQS) + ", " + str(item[0]) + " found " + str(G4_seq["start"]) + "-" + str(G4_seq["end"]))
                        found = True
                        break
...
def compare_lists_G4seq():
    """For each G4-seq window, determine whether its location overlaps with a PQS predicted by G4 Hnter, and store this
    information in G4seq_found_by_algorithm.
    """
    for G4_seq in G4seqs:  # loop over all of the G4 seq windows on the sense strand
    ...
    for G4_seq in G4seqs_reverse:  # loop over all of the G4-seq windows on the reverse complement strand
        ...
        for PQS in PQS_dict_RC:  # loop over all of the PQSs on the reverse complement strand
            for item in PQS_dict_RC[PQS]:
                if item[0] == G4_seq["chromosome"]:  # only consider G4-seq windows and PQSs on the same chromosome
                    if int(G4_seq["start"]) <= int(PQS) <= int(G4_seq["end"]):
                        G4seq_found_by_algorithm.append(str(PQS) + ", " + str(item[0]) + " found " + str(G4_seq["start"])
                                                        + "-" + str(G4_seq["end"]))
                        found = True
                        break
                    elif int(G4_seq["start"]) <= (int(PQS) + int(item[1])) <= int(G4_seq["end"]):
